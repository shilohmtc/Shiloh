...
@media(min-width:1100px){.category-discovery-grid{display:flex;flex-wrap:wrap;justify-content:center}.category-discovery-card{flex:0 1 calc((100% - 48px)/4)}}
@media(max-width:900px){.home-hero-grid,.story,.local-story{grid-template-columns:1fr}.client-portal-card{grid-template-columns:1fr;align-items:start}.client-portal-actions{justify-content:flex-start}.hero-visual{min-height:340px}.local-story__image{min-height:300px}.grid,.fact-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.contact-panel{grid-template-columns:1fr}.reviews-rail{grid-auto-columns:minmax(300px,calc((100% - 16px)/2))}}
...
function serviceCategoryPresentation(category = '') {
  ...
  if (label === 'body & wellness') {
    return {
      visual: 'massage',
      visualLabel: 'Editorial massage preparation in Shiloh’s colour palette',
      copy: 'Explore supportive body and wellness services with clear timing and prices.',
    };
  }
  if (label === 'permanent makeup') {
    return {
      visual: 'permanent-makeup',
      visualLabel: 'Editorial permanent makeup tools in Shiloh’s colour palette',
  ...
  if (label === 'advanced aesthetics') {
    return {
      visual: 'advanced',
      visualLabel: 'Editorial advanced aesthetics preparation in Shiloh’s colour palette',
  ...
  if (/pedicure|foot|feet|nail|gel/.test(label)) {
    return {
      visual: 'foot',
      visualLabel: 'Editorial foot-care preparation in Shiloh’s colour palette',
  ...
  if (/facial|aesthetic|skin|brow|lash|wax/.test(label)) {
    return {
      visual: 'facial',
      visualLabel: 'Editorial facial and skincare preparation in Shiloh’s colour palette',
  ...
  return {
    visual: 'other',
    visualLabel: 'Editorial self-care arrangement in Shiloh’s colour palette',
...
function townCentreNeighbourList() {
  return `<ul class="neighbour-list" aria-label="Businesses near Shiloh">${SHILOH_TOWN_CENTRE_NEIGHBOURS.map((name) => {
    const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${name}, Heidelberg, Gauteng`)}`;
    return `<li><a href="${escapeHtml(mapsUrl)}" target="_blank" rel="noopener noreferrer" aria-label="Open ${escapeHtml(name)} in Google Maps (opens in a new tab)">${escapeHtml(name)} <span aria-hidden="true">↗</span></a></li>`;
  }).join('')}</ul>`;
}
...
function renderHome(catalogue = [], options = {}) {
  ...
  return layout({
    ...
    currentPath: '/',
    body: `<main id="main-content"><section class="home-hero"><div class="shell home-hero-grid"><div><div class="eyebrow">In the heart of Heidelberg</div><h1>Feel cared for, from the first hello.</h1><p class="lede">Massage and aesthetic services at ${escapeHtml(SHILOH_PUBLIC_LOCATION.streetAddress)}, in Heidelberg’s newly renovated town centre. Explore what suits you and continue into one simple booking journey when you are ready.</p><div class="actions"><a class="button primary" href="/book">View services &amp; book</a><a class="button secondary" href="/about">Meet Shiloh</a></div></div><div class="hero-visual" role="img" aria-label="Editorial wellness arrangement in Shiloh’s colour palette"><div class="hero-visual-card"><div class="eyebrow">The Shiloh feeling</div><h3>${escapeHtml(PUBLIC_TAGLINE)}</h3><p>Calm, considered care in Heidelberg.</p></div></div></div></section><section class="section alt" data-public-service-discovery><div class="shell"><div class="section-head"><div><div class="eyebrow">Find what feels right</div><h2>What would you like today?</h2></div><p>Choose a category to explore Shiloh’s current services, timing and prices. Everything comes from the same live catalogue used for booking.</p></div><div class="category-discovery-grid">${discoveryMarkup}</div><div class="actions"><a class="button primary" href="/treatments">View all current services</a><a class="button secondary" href="/book#choose-with-shiloh">Not sure? Ask Shiloh</a></div></div></section>${googleReviewsSection(options.reviewPreview)}<section class="section"><div class="shell"><div class="local-story" data-public-visit-shiloh><div class="local-story__image" role="img" aria-label="Editorial welcome image inspired by Shiloh’s colour palette"></div><div class="local-story__copy"><div class="eyebrow">Come visit Shiloh</div><h2>Come experience the new heart of Heidelberg.</h2><p>Shiloh is situated at ${escapeHtml(SHILOH_PUBLIC_LOCATION.streetAddress)}, in the heart of Heidelberg’s newly renovated town centre. ${escapeHtml(STANDARD_HOSPITALITY)} Then enjoy the independent food, coffee and local businesses surrounding us.</p>${townCentreNeighbourList()}<small class="local-story__note">Our neighbours are independent local businesses. Please confirm their current details directly.</small><div class="actions"><a class="button primary" href="/contact">Plan your visit</a><a class="button secondary" href="/visit">Explore Heidelberg</a></div></div></div></div></section><section class="client-portal" data-my-shiloh-install-entry><div class="shell"><div class="client-portal-card"><div><div class="eyebrow">My Shiloh</div><h2>Keep Shiloh one tap away.</h2><p>Add My Shiloh to your Home Screen for quick access to bookings, Wallet, notifications and your private client space.</p><small>No App Store or Play Store download required. We’ll show the right steps for your phone.</small></div><div class="client-portal-actions"><a class="button primary" href="/my-shiloh/" data-my-shiloh-install-link>Install My Shiloh</a><a class="button secondary" href="/my-shiloh/">Open My Shiloh</a></div></div></div></section>${signatureBlock()}</main>`,
  });
}
