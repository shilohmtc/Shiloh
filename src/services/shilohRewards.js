...
function createShilohRewardsService({db=pool,notifyClient=null}={}) {
  ...
  async function depositPosition(queryable,account) {
    if(!account)return null;
    const row=(await queryable.query(`SELECT req.required_amount,
      ...
      COALESCE((SELECT SUM(d.retained_amount) FROM booking_deposit_dispositions d WHERE d.requirement_id=req.id),0) AS retained
      FROM booking_payment_requirements req WHERE req.payment_account_id=$1 LIMIT 1`,[account.id])).rows[0];
    if(!row)return null;
    const credited=Math.max(0,Number(row.net_paid||0)-Number(row.retained||0));
    return {required:Number(row.required_amount||0),credited,remaining:Math.max(0,Number(row.required_amount||0)-credited)};
  }

  async function assertDepositSatisfied(queryable,account) {
    const deposit=await depositPosition(queryable,account);
    if(deposit&&deposit.remaining>0)throw new ShilohRewardsError('REWARDS_DEPOSIT_REQUIRED','The booking deposit must be paid before Shiloh Rewards can be applied.',409);
  }

  async function accountOutstanding(queryable,account,due) {
    if(!account)return due;
    const row=(await queryable.query(`SELECT COALESCE((SELECT SUM(CASE WHEN entry_type='payment' THEN amount ELSE -amount END) FROM payment_ledger_entries WHERE payment_account_id=$1),0) AS net_paid,COALESCE((SELECT SUM(amount) FROM booking_loyalty_allocations WHERE booking_payment_account_id=$1 AND state='applied'),0) AS rewards,COALESCE((SELECT SUM(amount) FROM booking_welcome_voucher_allocations WHERE booking_payment_account_id=$1 AND state='applied'),0) AS welcome,COALESCE((SELECT SUM(d.retained_amount) FROM booking_deposit_dispositions d JOIN booking_payment_requirements req ON req.id=d.requirement_id WHERE req.payment_account_id=$1),0) AS retained`,[account.id])).rows[0];
    return Math.max(0,Number(due)-Math.max(0,Number(row.net_paid||0)-Number(row.retained||0))-Number(row.rewards||0)-Number(row.welcome||0));
  }
