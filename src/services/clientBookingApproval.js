...
async function createPendingBookingApproval(db, { appointmentId, occasionNote = null, specialOccasion = null }) {
  ...
  const result = await db.query(`
    INSERT INTO appointment_booking_approvals (
      ...
      requested_starts_at,requested_ends_at,requested_revision,client_occasion_note,client_special_occasion
    )
    SELECT a.id,ast.staff_id,'pending','standard',a.client_id,a.crm_v2_client_id,
           ...
           a.location_id,ast.staff_id,staff_snapshot.ids,aps.service_id,service_snapshot.ids,a.starts_at,a.ends_at,a.updated_at,$2,$3
      FROM appointments a
      JOIN appointment_staff ast ON ast.appointment_id=a.id AND ast.position=1
      JOIN appointment_services aps ON aps.appointment_id=a.id AND aps.position=1
      JOIN LATERAL (SELECT array_agg(x.staff_id ORDER BY x.position,x.id) AS ids FROM appointment_staff x WHERE x.appointment_id=a.id) staff_snapshot ON TRUE
    ...
    ON CONFLICT (appointment_id) DO UPDATE SET
      ...
      client_special_occasion=EXCLUDED.client_special_occasion,
      planning_started_at=NULL,
      planning_by_admin_id=NULL,
      updated_at=NOW()
    WHERE appointment_booking_approvals.status='pending'
    RETURNING *`, [positiveId(appointmentId), occasionNote, specialOccasion]);
  return result.rows?.[0] || null;
}
