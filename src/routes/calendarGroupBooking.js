...
const { createCalendarGroupBookingService } = require('../services/calendarGroupBooking');
const { renderCalendarGroupBookingPage, calendarGroupBookingClientScript } = require('../presentation/calendarGroupBookingUx');
...
function errorStatus(error) {
  ...
  if (String(error?.code || '').startsWith('GROUP_') || String(error?.code || '').startsWith('LINKED_')) return 400;
  return 503;
}

function createCalendarGroupBookingRouter({
  env = process.env,
  sessionService,
  bookingService = createCalendarGroupBookingService({ db: pool }),
  renderPage = renderCalendarGroupBookingPage,
  renderClient = calendarGroupBookingClientScript,
} = {}) {
  if (!sessionService) throw new Error('Group booking staff session service is required');
  ...
  router.get('/', requireSession, async (req, res, next) => {
    try {
      ...
      return res.status(200).type('html').send(renderPage({ options, prefill, clientScriptPath: `${req.baseUrl || '/calendar/book/group'}/client.js` }));
    } catch (error) {
  ...
  router.post('/client-search', sameOrigin, requireSession, async (req, res, next) => {
    try { return res.status(200).json(await bookingService.searchClients(req.staffBrowserSession.adminId, String(req.body?.query || ''))); }
    catch (error) {
      const status = errorStatus(error);
...
module.exports = { createCalendarGroupBookingRouter, errorStatus };
