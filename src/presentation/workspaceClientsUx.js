...
function renderClientDetailPage(model, {
  ...
  const historyRows = (model.appointments || []).map(appointment => {
    ...
    return `<article class="history-row${cancelled}"><div class="history-time"><strong>${escapeHtml(start.date)}</strong><small>${escapeHtml(start.time)}${end.time ? `–${escapeHtml(end.time)}` : ''}</small></div><div class="history-service"><span class="history-service-name">${escapeHtml(appointmentServices(appointment))}</span><small class="booking-readiness">${escapeHtml(readinessText(readiness))}</small></div><div class="history-staff">${escapeHtml(appointmentStaff(appointment))}</div>${statusPill(appointment.status)}</article>`;
  }).join('');
