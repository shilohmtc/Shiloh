...
function acceptanceChannelLabel(value) {
  const channel = String(value || '').toLowerCase();
  if (channel === 'clinic_device') return 'In clinic on Shiloh device';
  if (channel === 'payment_link') return 'Secure link on client phone';
  if (channel === 'my_shiloh') return 'My Shiloh';
  if (channel === 'whatsapp') return 'WhatsApp';
  return channel ? channel.replace(/_/g, ' ') : 'Recorded';
}

function readinessMarkup(readiness) {
  if (!readiness) return '';
  const deposit = readiness.depositState === 'satisfied' ? ['Deposit received','ok']
    : readiness.depositState === 'exempt' ? ['No deposit required','ok']
      : readiness.depositState === 'awaiting' ? ['Deposit awaiting','wait']
        : ['Deposit not recorded','wait'];
  return `<div class="booking-readiness" aria-label="Booking readiness">
    <span class="readiness-chip ${readiness.termsAccepted ? 'ok' : 'wait'}">${readiness.termsAccepted ? '✓ Terms accepted' : 'Terms awaiting'}</span>
    <span class="readiness-chip ${deposit[1]}">${escapeHtml(deposit[0])}</span>
    <span class="readiness-chip ${readiness.confirmed ? 'ok' : 'wait'}">${readiness.confirmed ? '✓ Confirmed' : 'Confirmation awaiting'}</span>
  </div>`;
}
...
function renderClientDetailPage(model, {
  ...
  const policyRows = (model.policyAcceptances || []).map(item => {
    ...
    const booking = item.appointment_id ? `Booking #${item.appointment_id}` : (item.service_text || 'Booking request');
    return `<div class="policy-history-row"><strong>${escapeHtml(item.policy_version || 'Policy')}</strong><span>${escapeHtml(accepted.date)} ${escapeHtml(accepted.time)}</span><span>${escapeHtml(acceptanceChannelLabel(item.channel))} · ${escapeHtml(booking)}</span></div>`;
  }).join('');
  return `${shellStart({ title: 'Client detail', subtitle: 'Client profile and appointment history.', displayName: model.authority?.displayName, calendarNavigationAllowed, staffAccessScriptPath })}<main data-client-detail-view>
    ...
    <section class="history-panel" data-policy-acceptance-history style="margin-bottom:12px"><header class="section-heading"><div><span class="eyebrow">Policies &amp; consents</span><h2>Booking Policy &amp; Terms</h2></div><span class="truth-note">Exact version and acceptance channel</span></header><div class="policy-history-list">${policyRows || '<div class="empty">No Booking Policy acceptance is recorded for this client yet.</div>'}</div></section>
