...
<main class="shell" data-my-shiloh-booking data-csrf="${escapeHtml(csrfToken)}" data-selected-service-id="${escapeHtml(selectedServiceId)}" data-deposit-rate="${escapeHtml(rate)}" data-deposit-free-hours="${escapeHtml(freeHours)}" data-deposit-partial-hours="${escapeHtml(partialHours)}" data-deposit-partial-forfeit="${escapeHtml(partialForfeit)}" data-deposit-late-forfeit="${escapeHtml(lateForfeit)}">
...
<div class="notice"><strong>Booking deposit:</strong> ${escapeHtml(rate)}% is required after Shiloh approves your booking request. Cancellation terms: ${escapeHtml(freeHours)}+ hours no penalty; ${escapeHtml(partialHours)}–${escapeHtml(freeHours)} hours may forfeit ${escapeHtml(partialForfeit)}% of the deposit; under ${escapeHtml(partialHours)} hours or a no-show may forfeit ${escapeHtml(lateForfeit)}%.</div>${voucherNote}<div class="notice"><strong>Need a Couples Massage, package session or a treatment with final pricing?</strong> <a href="/my-shiloh/#shiloh">Ask Shiloh in the app</a> and we’ll guide you through the right booking path.</div></section>
...
</main><script src="/my-shiloh/assets/booking.js?v=20260926-occasion-choice-v2" defer></script></body></html>`;
}
