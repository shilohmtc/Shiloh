...
<div class="notice"><strong>Booking deposit:</strong> ${escapeHtml(rate)}% is required after Shiloh approves your booking request. Marietjie’s appointments are deposit-exempt. Cancellation terms: ${escapeHtml(freeHours)}+ hours no penalty; ${escapeHtml(partialHours)}–${escapeHtml(freeHours)} hours may forfeit ${escapeHtml(partialForfeit)}% of the deposit; under ${escapeHtml(partialHours)} hours or a no-show may forfeit ${escapeHtml(lateForfeit)}%.</div>${voucherNote}<div class="notice"><strong>Need a Couples Massage, package session or a treatment with final pricing?</strong> <a href="/my-shiloh/#shiloh">Ask Shiloh in the app</a> and we’ll guide you through the right booking path.</div></section>
