...
function welcomeVoucherDashboardPanel(campaign) {
  if (!campaign) return '';
  return `<section class="panel" data-dashboard-voucher-panel><header class="panel-head"><div><span class="eyebrow">Private campaign</span><h2>R100 Welcome Voucher</h2><p class="truth-note">Since launch · Christel and JP only</p></div></header><div class="voucher-dashboard-metrics"><div class="voucher-dashboard-metric"><strong>${escapeHtml(campaign.vouchersUnlocked || 0)}</strong><span>Unlocked</span></div><div class="voucher-dashboard-metric"><strong>${escapeHtml(campaign.vouchersRedeemed || 0)}</strong><span>Redeemed · ${escapeHtml(campaign.redemptionPercent || 0)}%</span></div><div class="voucher-dashboard-metric"><strong>${escapeHtml(campaign.expiringSoon || 0)}</strong><span>Expire in 7 days</span></div></div><div class="voucher-dashboard-foot"><p>${escapeHtml(formatRand(campaign.discountsGiven))} in welcome discounts applied.</p><a class="button" href="/calendar/reports#welcome-voucher">View report</a></div></section>`;
}

function bookingRequestItem(item, model) {
  const awaiting = item.effectiveStatus === 'awaiting_client_confirmation';
  const status = awaiting ? 'Awaiting client response' : item.planningStartedAt ? 'Planning · Reception' : 'Requested · Reception planning';
