...
function renderFormsPage(model = {}) {
  ...
  const cards = (model.templates || []).map(form => {
    ...
    const href = `/calendar/forms/${encodeURIComponent(String(form.templateKey || ''))}`;
    return `<a class="form-card" href="${escapeHtml(href)}" data-form-template="${escapeHtml(form.templateKey)}" aria-label="Open ${escapeHtml(form.title)} preview">
      ...
      <div class="open-row">Open form preview →</div>
    </a>`;
  }).join('');
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Forms — Shiloh Workspace</title><style>${workspaceShellStyles()}${formsStyles()}</style></head><body data-workspace-forms="true"><div class="workspace-frame">${renderWorkspaceNavigation({
    ...
    <section class="notice" aria-label="Forms setup status"><div><strong>The form library is ready.</strong><p>Tap a form below to preview its questions. The secure client submission page and automatic WhatsApp sending will be connected next.</p></div></section>
...
function renderFormsUnavailablePage({ message = 'Forms are temporarily unavailable.' } = {}) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Forms — Shiloh Workspace</title><style>${workspaceShellStyles()}${formsStyles()}</style></head><body><div class="workspace-frame">${renderWorkspaceNavigation({ active: 'forms', formsHref: '/calendar/forms' })}<main class="workspace-main"><div class="shell"><header class="topbar"><div class="brand"><h1>Forms</h1><p>${escapeHtml(message)}</p></div></header></div></main></div></body></html>`;
}
