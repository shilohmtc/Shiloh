...
body[data-calendar-desktop-approved="true"] .desktop-create-popover>a[data-calendar-booking-kind="couples"]{background:#fbf5f8;color:#633f55}
body[data-calendar-desktop-approved="true"] .desktop-create-popover>a[data-calendar-booking-kind="couples"]:hover{background:#f4e8ee}
body[data-calendar-desktop-approved="true"] .desktop-create-popover>a[data-calendar-booking-kind="couples"]::after{content:'2 guests';margin-left:auto;padding:2px 6px;border-radius:999px;background:#ead9e1;color:#633f55;font-size:.62rem;font-weight:850;white-space:nowrap}
body[data-calendar-desktop-approved="true"] .desktop-create-popover>a[data-calendar-booking-kind="group"]{background:#f2f0fa;color:#4f496f}
body[data-calendar-desktop-approved="true"] .desktop-create-popover>a[data-calendar-booking-kind="group"]::after{content:'3–10 guests';margin-left:auto;padding:2px 6px;border-radius:999px;background:#dfdbf1;color:#4f496f;font-size:.62rem;font-weight:850;white-space:nowrap}
body[data-calendar-desktop-approved="true"] .desktop-create-popover>a[data-calendar-booking-kind="multiple"]{background:#edf4ef;color:#234f3b}
body[data-calendar-desktop-approved="true"] .desktop-create-popover>a[data-calendar-booking-kind="multiple"]::after{content:'1 client';margin-left:auto;padding:2px 6px;border-radius:999px;background:#d9e8de;color:#234f3b;font-size:.62rem;font-weight:850;white-space:nowrap}
body[data-calendar-desktop-approved="true"] .desktop-create-divider{height:1px;margin:4px 5px;background:var(--line)}
...
function calendarDesktopApprovedClientScript() {
  ...
  const icons = JSON.stringify({
    plus: iconSvg('plus'), calendarPlus: iconSvg('calendarPlus'), couples: iconSvg('couples'), users: iconSvg('users'), history: iconSvg('history'), block: iconSvg('block'),
...
function buildToolbar(days,selected){const controls=one('.controls');if(!controls)return null;const period=one('.period-nav',controls),tabs=one('.view-tabs',controls),today=one('.today',controls);if(!period||!tabs||!today)return null;const teamDay=one('[data-calendar-view-option="week"]',tabs);if(teamDay){teamDay.textContent='Team day';teamDay.setAttribute('aria-label','Team day calendar');}const context=one('.period-context',period);if(context&&days.length)context.textContent=compactLabel(days);const arrows=all('.period-arrow',period);if(arrows[0])arrows[0].innerHTML=ICONS.chevronLeft;if(arrows[1])arrows[1].innerHTML=ICONS.chevronRight;today.href=calendarHref('week',businessToday());iconizeLink(today,'today','Today');const toolbar=document.createElement('section');toolbar.className='desktop-calendar-toolbar';toolbar.setAttribute('aria-label','Calendar navigation');toolbar.append(period,tabs,today);const menu=document.createElement('details');menu.className='desktop-create-menu';menu.innerHTML='<summary aria-label="Book or add calendar item">'+ICONS.plus+'<span>Book</span></summary><div class="desktop-create-popover" data-desktop-create-popover></div>';toolbar.append(menu);controls.parentNode.insertBefore(toolbar,controls);const popover=one('[data-desktop-create-popover]',menu);all('.operational-actions a').forEach(link=>{const clone=link.cloneNode(true);clone.className='';const isPast=/Record past/i.test(link.textContent);iconizeLink(clone,isPast?'history':'calendarPlus',isPast?'Record past appointment':'New appointment');popover.append(clone);if(!isPast){const multiple=document.createElement('a');multiple.href='/calendar/book/multiple?'+new URLSearchParams({date:selected}).toString();multiple.dataset.calendarBookingKind='multiple';multiple.setAttribute('aria-label','Book multiple treatments for one client');iconizeLink(multiple,'calendarPlus','Multiple treatments');popover.append(multiple);const couples=document.createElement('a');couples.href='/calendar/book/couples?'+new URLSearchParams({date:selected}).toString();couples.dataset.calendarBookingKind='couples';couples.setAttribute('aria-label','Book a Couples Massage for two guests');iconizeLink(couples,'couples','Couples massage');popover.append(couples);const group=document.createElement('a');group.href='/calendar/book/group?'+new URLSearchParams({date:selected}).toString();group.dataset.calendarBookingKind='group';group.setAttribute('aria-label','Book a Group booking for multiple guests');iconizeLink(group,'users','Group booking');popover.append(group);}});all('.operational-actions').forEach(node=>node.remove());return {menu,popover};}
...
async function populateAvailability(popover,selected){if(!popover||body.dataset.calendarReadonly==='true')return;let source=document;if(!one('[data-calendar-operation="add-block"], [data-calendar-operation="add-leave"]')){try{const url=new URL(location.href);url.searchParams.set('view','day');url.searchParams.set('date',selected);const response=await fetch(url.pathname+'?'+url.searchParams.toString(),{credentials:'same-origin',cache:'no-store',headers:{Accept:'text/html'}});if(!response.ok)return;source=new DOMParser().parseFromString(await response.text(),'text/html');}catch(_error){return;}}const allowed=collectAvailability(source);if(allowed.block.size||allowed.leave.size){const divider=document.createElement('div');divider.className='desktop-create-divider';divider.setAttribute('role','separator');divider.setAttribute('aria-label','Availability controls');popover.append(divider);}appendAvailability(popover,'Block time','add-block',allowed.block,'block');appendAvailability(popover,'Leave','add-leave',allowed.leave,'leave');}
installStyles();const days=extractDays();const selected=selectedDate(days);const toolbar=buildToolbar(days,selected);buildWeekStrip(days,selected,one('.week-view'));buildPlanner(selected);if(toolbar)populateAvailability(toolbar.popover,selected);body.removeAttribute('data-calendar-desktop-pending');
})();`;
}
