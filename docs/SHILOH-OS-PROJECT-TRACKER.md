...
|---|---|---|---|
| MY-SHILOH-AI-BACKBONE | Client Experience / Shiloh AI | 🔵 ACTIVE | Phase 1 client context/orchestrator is VERIFIED LIVE under #1037; Phase 2 authenticated in-app Shiloh is VERIFIED LIVE under #1038 / `bd4b580b...`. Current controlled unit is Phase 3 typed read tools: next appointment, upcoming bookings, consultation-form status, payment position and canonical client availability. Tools are bound to the authenticated CRM V2 session, expose no client selector, perform no mutation and return presentation-safe facts only. |
