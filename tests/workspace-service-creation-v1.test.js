...
function fakeDb({ role = 'booking_operator', linkedStaffId = null, practitionerIds = [11], tenantOwner = null, duplicate = false } = {}) {
  ...
  const client = {
    async query(sql, values = []) {
      ...
      if (sql.includes('INSERT INTO services(')) return { rows: [{ id: 99, category_id: Number(values[0]), name: values[1], duration_minutes: Number(values[2]), processing_time_minutes: 0, extra_time_minutes: 0, variable_price: values[3], price: values[4], display_price: values[5], customer_description: values[6], status: 'active' }] };
...
test('creation pricing validates fixed and variable price policy explicitly', () => {
  ...
  assert.throws(() => normalizeCreatePayload({ name: 'A', categoryId: 3, durationMinutes: 60, staffIds: [1], price: '', variablePrice: false, customerDescription: description }), /requires a price/i);
  assert.throws(() => normalizeCreatePayload({ name: 'A', categoryId: 3, durationMinutes: 0, staffIds: [1], price: 100, variablePrice: false, customerDescription: description }), /between 1 and 1440/i);
  assert.throws(() => normalizeCreatePayload({ name: 'A', categoryId: 3, durationMinutes: 60, staffIds: [1], price: 100, variablePrice: false, customerDescription: 'Too short' }), /between 20 and 2000/i);
  assert.throws(() => normalizeCreatePayload({ name: 'A', categoryId: '', durationMinutes: 60, staffIds: [1], price: 100, variablePrice: false, customerDescription: description }), /valid service category/i);
  const variable = normalizeCreatePayload({ name: 'Variable', categoryId: 3, durationMinutes: 45, staffIds: [1], price: '', displayPrice: 'From R500', variablePrice: true, customerDescription: description });
  assert.equal(variable.categoryId, 3);
...
test('tenant practitioner creates only own canonical tenant-private service', async () => {
  ...
  const result = await service.createService({ adminId: 40, requestId: 'request_1234', name: 'Tenant Custom', categoryId: 3, durationMinutes: 50, price: '550', variablePrice: false, customerDescription: 'A tailored service with clear client-facing information.', staffIds: [11] });
  assert.equal(result.status, 'created');
  assert.equal(result.service.privateOwnerStaffId, 11);
  assert.equal(result.service.categoryId, 3);
  assert.equal(result.service.categoryName, 'Massage');
  ...
  await assert.rejects(
    createWorkspaceServiceCreationService({ db: denied.db }).createService({ adminId: 40, requestId: 'request_5678', name: 'Wrong Target', categoryId: 3, durationMinutes: 50, price: '550', variablePrice: false, customerDescription: 'A tailored service with clear client-facing information.', staffIds: [12] }),
...
test('booking operator can create an ordinary shared service without gaining services:manage', async () => {
  ...
  const service = createWorkspaceServiceCreationService({ db });
  const result = await service.createService({ adminId: 40, requestId: 'request_shared', name: 'Shared Custom', categoryId: 3, durationMinutes: 30, price: '300', variablePrice: false, customerDescription: 'A shared service ready for clients to discover online.', staffIds: [11, 12] });
...
test('duplicate canonical service name fails closed before any service write', async () => {
  ...
  await assert.rejects(
    createWorkspaceServiceCreationService({ db }).createService({ adminId: 40, requestId: 'request_duplicate', name: 'Existing', categoryId: 3, durationMinutes: 60, price: '600', variablePrice: false, customerDescription: 'An existing service description that will not be saved.', staffIds: [11] }),
...
test('route and UX use one guarded creation endpoint from Workspace and Calendar', () => {
  ...
  assert.match(ux, /data-create-category/);
  assert.match(ux, /categoryId/);
