...
test('Storybook workspace catalogue consumes production presentation authorities', () => {
  ...
  assert.match(story, /BookingConflictRecovery/);
  assert.match(story, /recovery-detail/);
});
...
test('visual gate covers each workspace surface on Desktop and Phone', () => {
  ...
  assert.match(visual, /workspace-production-surfaces--booking-conflict-recovery/);
